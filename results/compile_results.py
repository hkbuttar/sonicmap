"""Step 11: compile Steps 4–10 into an honest, reproducible report."""
...
def compile_results(root: Path):
    root = Path(root)
    step4_raw = pd.read_csv(root / "classification/results/step4_classification.csv")
    step4 = pd.read_csv(root / "classification/results/step4_classification_summary.csv")
    mood_raw = pd.read_csv(root / "mood/results/step5_mood_regression.csv")
    mood = pd.read_csv(root / "mood/results/step5_mood_regression_summary.csv")
    step8 = pd.read_csv(root / "similarity/results/step8_similarity_comparison.csv")
    step9_class = pd.read_csv(root / "generalization/results/step9_classification_generalization.csv").iloc[0]
    step9_similarity = pd.read_csv(root / "generalization/results/step9_similarity_generalization.csv")
    ...
    for model in ("cnn", "gbm"):
        values = step4_raw[step4_raw["model"] == model].pivot(index="fold", columns="augmented", values="accuracy")
        delta, low, high = mean_ci(values[True] - values[False])
        rows.append(_row("augmentation", model, "accuracy_delta", delta, low, high,
                         "step4 fold-paired CV", "Positive values favor augmentation."))
    for model in ("cnn", "gbm"):
        record = step4[(step4["model"] == model) & (step4["augmented"] == True) & (step4["metric"] == "accuracy")].iloc[0]
        rows.append(_row("genre_classification", model, "accuracy", record["mean"], record["ci_low"], record["ci_high"], "step4 5-fold CV"))

    for target in ("valence", "arousal"):
        metric = f"{target}_r2"
        for model in ("cnn", "gbm"):
            record = mood[(mood["model"] == model) & (mood["metric"] == metric)].iloc[0]
            rows.append(_row("mood_regression", model, metric, record["mean"], record["ci_low"], record["ci_high"], "step5 5-fold CV"))
        ...
        rows.append(_row("mood_regression", "gbm_minus_cnn", f"{target}_r2_delta", delta, low, high,
                         "step5 fold-paired CV", "Positive values favor GBM."))

    rows.extend([
        _row("embedding", "classification_embedding", "cosine_silhouette", classification_embedding["embedding_silhouette_cosine"], source="step6"),
        _row("embedding", "triplet_embedding", "cosine_silhouette", triplet_embedding["embedding_silhouette_cosine"], source="step7"),
        _row("embedding", "triplet_training", "triplet_loss_reduction",
             triplet_embedding["initial_triplet_loss"] - triplet_embedding["final_triplet_loss"], source="step7"),
    ])
    for method in ("classification_embedding", "triplet_embedding", "engineered_features", "metadata_genre"):
        record = step8[(step8["method"] == method) & (step8["k"] == 10)].iloc[0]
        ...
        rows.append(_row("similarity", method, "precision_at_10", record["precision_at_k"], record["ci_low"], record["ci_high"], "step8 GTZAN in-sample retrieval", caveat))

    rows.append(_row("generalization", "genre_cnn", "fma_accuracy", step9_class["accuracy"], step9_class["accuracy_ci_low"], step9_class["accuracy_ci_high"], "step9 FMA exact-overlap", "Only three exact-overlap genres."))
    rows.append(_row("generalization", "genre_cnn", "absolute_accuracy_drop", step9_class["absolute_accuracy_drop"], source="GTZAN CV minus FMA"))
    for method in ("classification_embedding", "triplet_embedding"):
        record = step9_similarity[(step9_similarity["method"] == method) & (step9_similarity["k"] == 10)].iloc[0]
        rows.append(_row("generalization", method, "fma_precision_at_10", record["fma_precision_at_k"], record["fma_ci_low"], record["fma_ci_high"], "step9 FMA retrieval"))
        rows.append(_row("generalization", method, "chance_adjusted_precision_drop_at_10", record["adjusted_precision_drop"], source="step9", caveat="Adjusts for 3 FMA versus 10 GTZAN classes."))

    for method in ("classification_embedding", "triplet_embedding"):
        record = playlists[(playlists["method"] == method) & (playlists["metric"] == "pairwise_similarity")].iloc[0]
        rows.append(_row("playlist", method, "pairwise_similarity_lift_vs_random", record["mean_difference"], record["difference_ci_low"], record["difference_ci_high"], "step10 100 seeds", "Compare lifts, not raw cosine levels across spaces."))
...
def render_report(summary: pd.DataFrame) -> str:
    ...
    return f"""# Step 11 — Results and Honest Comparison
...
def main() -> int:
    ...
    summary.to_csv(args.output_dir / "step11_comparison.csv", index=False)
    report = render_report(summary)
    (args.output_dir / "step11_findings.md").write_text(report)
    print(report)
    return 0
