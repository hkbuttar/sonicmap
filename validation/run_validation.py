"""Step 12: run the complete correctness and experiment sanity audit."""
...
def run(root: Path):
    ...
    for record in genre_accuracy.itertuples():
        checks.append(interval_vs_baseline_check(
            ...
            "Step 4 five-fold accuracy; uniform random baseline is 0.10.", "critical",
        ))
    ...
    for record in mood[mood.metric.str.endswith("_r2")].itertuples():
        checks.append(interval_vs_baseline_check(
            ...
            "Step 5 five-fold R²; zero is the mean-target baseline.", "critical",
        ))
    ...
    checks.append(Check(
        ...
        "Augmented variants included by the Step 4 mask originate only from training-fold tracks.",
    ))
    ...
    checks.append(greater_than_check(
        ...
        "Step 7 first versus final epoch mean triplet loss.",
    ))
    ...
    similarity = pd.read_csv(root / "similarity/results/step8_similarity_comparison.csv")
    ...
    for record in p10[p10.method != "metadata_genre"].itertuples():
        checks.append(interval_vs_baseline_check(
            ...
            "Step 8 GTZAN P@10; balanced-label random baseline is 0.10.", "critical",
        ))
    ...
    fma_result = pd.read_csv(root / "generalization/results/step9_classification_generalization.csv").iloc[0]
    fma_integrity = (
    ...
    checks.append(interval_vs_baseline_check(
        ...
        "Step 9 FMA accuracy; the interval overlaps the balanced three-class random baseline.", "informational",
    ))
    ...
    for record in playlists[playlists.metric == "pairwise_similarity"].itertuples():
        checks.append(interval_vs_baseline_check(
            ...
            "Step 10 paired lift over matched random playlists.", "critical",
        ))
...
def render_report(checks, untrained_silhouette):
    ...
    lines.extend([
        ...
        "",
        f"The seeded untrained triplet network had cosine silhouette {untrained_silhouette:.6f}; training raised it to the saved Step 7 value while also reducing triplet loss.",
        "",
...
def main() -> int:
    ...
    pd.DataFrame([check.to_dict() for check in checks]).to_csv(args.output_dir / "step12_validation_checks.csv", index=False)
    report = render_report(checks, untrained_silhouette)
    (args.output_dir / "step12_validation_report.md").write_text(report)
    print(report)
